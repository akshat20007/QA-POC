...
**Data flow (CLI, Phases 1-3):** a plain-English user story (`qa-poc/stories/*.txt`) → `generate.py` calls Gemini with a strict-JSON schema (see `JSON_SCHEMA`/`SYSTEM_PROMPT_BASE` in `generate.py`) → a `TestCase` JSON object with `steps[]` (each step: `type` given/when/then, `action` free text, `target_hint`, optional `value` for fills) → written to `output/*.json` → `qa-poc/executor/src/run.ts` loads every file there and runs it via `runner.ts`.

**Site context grounding:** before calling Gemini, `generate.py`'s `load_site_context()` reads `qa-poc/context/saucedemo.md` (a hand-produced site-exploration doc — real accessible role/name pairs, page structure, flow notes, caveats) and `build_system_prompt()` appends it to `SYSTEM_PROMPT_BASE` as a reference section, so generated `target_hint`s are grounded in the site's actual DOM/accessibility tree rather than guessed. Both `main()` and `main_single()` load it once per process. A missing context file is not fatal — it logs a warning to stderr and generation proceeds without the extra grounding.
